...
export class MtdProductionService {
  ...
  private buildMainQuery(startDate: string, endDate: string, skip: number, limit: number, unit_id?: string, shift?: string) {
    ...
    if (unit_id) {
      query.andWhere('rpbdp.population_id = :population_id', {
        population_id: unit_id,
      });
    }
    if (shift) {
      query.andWhere('rpbdp.shift = :shift', {
        shift: shift,
      });
    }
  ...
  private buildCountQuery(startDate: string, endDate: string, unit_id?: string, shift?: string) {
    ...
    if (unit_id) {
      query.andWhere('m.id = :unit_id', { unit_id });
    }
    if (unit_id) {
      query.andWhere('rpbdp.shift = :shift', { shift });
    }
  ...
  getMtdWorkHour(filters: MtdProductionQueryDto) {
    try {
      return {
        statusCode: 200,
        message: 'success',
  ...
  getDayWorkHour(filters: MtdProductionQueryDto) {
    try {
      return {
        statusCode: 200,
        message: 'success',
