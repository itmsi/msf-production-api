...
import { Repository, DataSource, In, IsNull, Between, SelectQueryBuilder } from 'typeorm';
...
export class ParentPlanWorkingHourService {
  ...
  private applyFilterExportData(qb: SelectQueryBuilder<any>, query: ExportParentPlanWorkingHourQueryDto): SelectQueryBuilder<any> {
  ...
  private mapExportDataToCsvRow(item: any, index: number) {
    ...
    const totalBreakdown = roundToTwoDecimals(parseFloat(item.total_breakdown) || 0);
    ...
    return {
      ...
      'EWH Hour/Month': roundToTwoDecimals(parseFloat(item.working_hour_month) || 0),
      'EWH Hour/Day': roundToTwoDecimals(parseFloat(item.working_hour_day) || 0),
      'Working Longshift (Day)': roundToTwoDecimals(parseFloat(item.working_day_longshift) || 0),
      'Working Hours Longshift': roundToTwoDecimals(parseFloat(item.working_hour_longshift) || 0),
