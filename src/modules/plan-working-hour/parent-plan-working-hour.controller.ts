...
export class ParentPlanWorkingHourController {
  ...
  async exportData(@Query() query: ExportParentPlanWorkingHourQueryDto, @Res({ passthrough: false }) res: Response) {
  ...
  downloadTemplate(): StreamableFile {
    try {
      const file = join(process.cwd(), 'src/modules/plan-working-hour/template-parent-plan-working-hour.csv');
      const stream = createReadStream(file);
      return new StreamableFile(stream, {
        type: 'text/csv',
        disposition: 'attachment; filename="template-parent-plan-working-hour.csv"',
      });
    } catch (error) {
  ...
  @ApiOperation({
    ...
    description:
      'Mengambil semua data parent plan working hour dengan perhitungan summary sesuai spesifikasi. Mendukung filter, sorting, dan pagination.',
  })
  @ApiQuery({
    name: 'page',
    required: false,
  ...
  @ApiOperation({
    ...
    description:
      'Mengambil detail data parent plan working hour berdasarkan rentang tanggal dengan perhitungan metrics lengkap. Mendukung filter dan pagination.',
  })
  @ApiQuery({
    name: 'start_date',
    required: false,
  ...
  @ApiOperation({
    ...
    description:
      'Mengupdate data parent plan working hour berdasarkan ID beserta semua tabel terkait (r_parent_plan_working_hour, r_plan_working_hour, r_plan_working_hour_detail)',
  })
  @ApiParam({
    name: 'id',
    description: 'ID parent plan working hour',
  ...
  @ApiBody({
    type: UpdateDetailParentPlanWorkingHourDto,
    description: 'Data detail parent plan working hour yang akan diupdate',
    examples: {
      example1: {
        summary: 'Contoh request update detail dengan activities',
        value: {
          plan_date: '2025-08-01',
  ...
  @ApiResponse({
    status: 200,
    description: 'Detail parent plan working hour berhasil diupdate',
    schema: {
      example: {
        statusCode: 200,
