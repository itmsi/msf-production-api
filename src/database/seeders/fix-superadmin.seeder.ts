...
export class FixSuperAdminSeeder {
  ...
  async run(): Promise<void> {
    ...
    try {
      ...
      const existingUser = await this.dataSource.query('SELECT id, username, password FROM m_user WHERE username = $1', ['superadmin']);
      ...
      if (existingUser.length === 0) {
        ...
        await this.dataSource.query('UPDATE m_user SET password = $1, "updatedAt" = NOW() WHERE username = $2', [
          passwordHash,
          'superadmin',
        ]);
      ...
      // Ensure role exists
      const existingRole = await this.dataSource.query('SELECT id FROM m_role WHERE role_code = $1', ['SUPER_ADMIN']);
      ...
      const userId = await this.dataSource.query('SELECT id FROM m_user WHERE username = $1', ['superadmin']);
      ...
      const existingUserRole = await this.dataSource.query('SELECT * FROM r_user_role WHERE user_id = $1 AND role_id = $2', [
        userId[0].id,
        existingRole[0].id,
      ]);
