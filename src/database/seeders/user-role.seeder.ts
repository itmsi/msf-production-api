...
export class UserRoleSeeder {
  ...
  async run(): Promise<void> {
    ...
    try {
      ...
      const superAdminUser = await this.dataSource.query('SELECT id FROM m_user WHERE username = $1', ['superadmin']);
      ...
      const existingRelation = await this.dataSource.query('SELECT * FROM r_user_role WHERE user_id = $1 AND role_id = $2', [
        userId,
        roleId,
      ]);
