...
export class FixQuarryTargetColumn1756354025246 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    const hasOldQuarryTarget = await queryRunner.hasColumn('r_parent_plan_production', 'total_quary_target');

    if (hasOldQuarryTarget) {
      // Rename the old column to the correct name
      await queryRunner.query(`ALTER TABLE "r_parent_plan_production" RENAME COLUMN "total_quary_target" TO "total_quarry_target"`);
      console.log('Successfully renamed total_quary_target to total_quarry_target');
    } else {
      // Check if the correct column already exists
      const hasCorrectQuarryTarget = await queryRunner.hasColumn('r_parent_plan_production', 'total_quarry_target');
