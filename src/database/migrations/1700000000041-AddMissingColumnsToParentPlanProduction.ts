...
export class AddMissingColumnsToParentPlanProduction1700000000041 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    const oldTableExists = await queryRunner.hasTable('TB_R_Parent_Plan_Production');
    ...
    const hasQuarryTarget = await queryRunner.hasColumn(tableName, 'total_quarry_target');
    const hasSisaStock = await queryRunner.hasColumn(tableName, 'total_sisa_stock');
    const hasFleet = await queryRunner.hasColumn(tableName, 'total_fleet');
    
    if (!hasQuarryTarget) {
      // Check if old column exists and rename it
      const hasOldQuarryTarget = await queryRunner.hasColumn(tableName, 'total_quary_target');
      if (hasOldQuarryTarget) {
        await queryRunner.query(`ALTER TABLE "${tableName}" RENAME COLUMN "total_quary_target" TO "total_quarry_target"`);
      } else {
        await queryRunner.query(`ALTER TABLE "${tableName}" ADD COLUMN "total_quarry_target" double precision NOT NULL DEFAULT 0`);
      }
    }
    
    if (!hasSisaStock) {
      await queryRunner.query(`ALTER TABLE "${tableName}" ADD COLUMN "total_sisa_stock" integer NOT NULL DEFAULT 0`);
    }
    
    if (!hasFleet) {
      await queryRunner.query(`ALTER TABLE "${tableName}" ADD COLUMN "total_fleet" integer NOT NULL DEFAULT 0`);
    }
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    const tableExists = await queryRunner.hasTable('r_parent_plan_production');
    if (tableExists) {
      await queryRunner.query(`ALTER TABLE "r_parent_plan_production" DROP COLUMN IF EXISTS "total_sisa_stock"`);
      await queryRunner.query(`ALTER TABLE "r_parent_plan_production" DROP COLUMN IF EXISTS "total_fleet"`);
      
      // Revert column name if it was renamed
      const hasQuarryTarget = await queryRunner.hasColumn('r_parent_plan_production', 'total_quarry_target');
      if (hasQuarryTarget) {
        await queryRunner.query(`ALTER TABLE "r_parent_plan_production" RENAME COLUMN "total_quarry_target" TO "total_quary_target"`);
      }
    }
  }
}
