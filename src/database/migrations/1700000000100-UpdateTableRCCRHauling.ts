...
export class UpdateTableRCCRHauling1700000000100 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE r_ccr_hauling ADD CONSTRAINT FK_r_ccr_hauling_dumping_point_op_id FOREIGN KEY (dumping_point_op_id) REFERENCES m_operation_points(id)`,
    );
    await queryRunner.query(
      `ALTER TABLE r_ccr_hauling ADD CONSTRAINT FK_r_ccr_hauling_dumping_point_barge_id FOREIGN KEY (dumping_point_barge_id) REFERENCES m_barge(id)`,
    );
  }
